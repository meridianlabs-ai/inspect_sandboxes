...
def make_env(
    project: NovitaDinDProject | None = None,
    service: str = "web",
    working_dir: str = "/app",
) -> NovitaDinDServiceEnvironment:
    if project is None:
        project = make_mock_project()
    return NovitaDinDServiceEnvironment(project, service, working_dir)
...
async def test_exec_routes_to_service_with_correct_command() -> None:
    ...
    with patch(
        "inspect_sandboxes.novita._dind_env.compose_exec",
...
async def test_exec_stdin_two_hop_upload() -> None:
    ...
    with (
        ...
        patch(
            "inspect_sandboxes.novita._dind_env.vm_exec",
            new_callable=AsyncMock,
            return_value=(0, "", ""),
        ),
    ):
...
@pytest.mark.asyncio
async def test_write_file_vm_upload_retries_transport_error() -> None:
    """The DinD VM write retries a leaked httpx transport error (via write_sandbox_file)."""
    sandbox = make_mock_sandbox()
    sandbox.files.write = AsyncMock(side_effect=[httpx.ReadError("reset"), None])
    env = make_env(make_mock_project(sandbox), service="web")
    env._is_directory = AsyncMock(return_value=False)  # type: ignore[method-assign]
...
async def test_sample_cleanup_destroys_and_kills() -> None:
    ...
    with patch(
        "inspect_sandboxes.novita._dind_env.destroy_dind_project",
        new_callable=AsyncMock,
    ) as mock_destroy:
...
async def test_sample_init_dind_serializes_compose_config() -> None:
    ...
    with (
        patch(
            "inspect_sandboxes.novita._dind_env.create_dind_project",
            new_callable=AsyncMock,
            return_value=mock_project,
        ) as mock_create,
        patch(
            "inspect_sandboxes.novita._dind_env.discover_working_dir",
            new_callable=AsyncMock,
            return_value="/",
        ),
    ):
