...
- **`stdout` / `stderr`**: Daytona's `process.exec()` API returns one merged output stream, so `exec()` runs commands through the sandbox's **session API** instead, which returns stdout, stderr and the exit status separately (the Daytona agent tags every output chunk with its stream). Each environment keeps a pool of sessions, one per concurrent `exec()`, and every command runs in its own `/bin/sh -c` with the working directory and environment applied inside it, so nothing leaks between calls; in DinD mode the `docker compose exec` runs in a session on the VM. A timeout is enforced inside the command with `/usr/bin/timeout -k 5s` (as the Docker sandbox does), so `/usr/bin/timeout` must exist in the image when `exec()` is given a timeout. Fidelity notes from the agent: a last line without a trailing newline comes back with one appended, invalid UTF-8 is replaced, NUL bytes and a background child's late output are kept. Measured on the default image: a session `exec()` costs about 25 ms more than `process.exec()`, and large outputs are noticeably slower (5 MiB in about 8 s against 0.65 s).
