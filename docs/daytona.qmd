...
- **`stdout` / `stderr`**: The Daytona API returns a single merged output stream with its trailing newline stripped, so `exec()` runs the command with both streams redirected to private (`0600`) temp files under `/tmp` — in the sandbox, as the requested `user`, for single-service mode; on the DinD VM for multi-service mode — and the same shell then prints both files framed by per-call sentinels, in one API round trip. `stdout` and `stderr` come back separately and byte for byte (the command cannot write into the stderr segment, as inspect_ai's framework-directory helper — sandbox tools, sandbox service, human agent, checkpointing — requires), at the cost of `exec()` needing a writable `/tmp` in the sandbox. The wrapper's own `cat`/`rm` are resolved via `/usr/sbin:/usr/bin:/sbin:/bin`, never the image's `PATH`.
