...
- **`stderr`**: The Daytona API returns a single merged output stream, so `exec()` redirects the command's stderr to a private (`0600`) temp file under `/tmp` — in the sandbox for single-service mode, on the DinD VM for multi-service mode — and reads it back with a second API call once the command has finished. `stdout` and `stderr` therefore come back separately, as inspect_ai's framework-directory helper (sandbox tools, sandbox service, human agent, checkpointing) requires. Each `exec()` costs two API round trips, and single-service images need a writable `/tmp`.
...
- **Trailing newline on `stdout`**: The Daytona API strips the trailing newline from the command output, so `stdout` comes back without it (`stderr`, read back through the temp file, is intact).
