...
- **Internet access**: Outbound internet access depends on your Daytona subscription tier. Tiers 1–2 restrict traffic to an "Essential Services" allowlist managed by Daytona, which includes container registries (Docker Hub, GHCR), package managers (npm, PyPI), and git hosts — so image pulls and package installs work on all tiers. Tiers 3–4 have full unrestricted internet access. The `network_block_all` and `network_allow_list` parameters cannot override tier-level restrictions. If tasks need arbitrary external endpoints (APIs, databases, etc.), a Tier 3+ subscription is required.
