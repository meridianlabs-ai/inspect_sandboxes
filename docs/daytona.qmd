...
- **`user` parameter**: Supported via `sudo -u` in single-service mode and `docker compose exec --user` in DinD mode. Numeric UIDs are supported. Requires `sudo` with passwordless access (configured by default). In single-service mode, `env` is passed through with `sudo --preserve-env`, which needs sudo 1.8.21 or later and a sudoers rule that allows it (the default rule does). Anything `sudo` itself prints (such as `unable to resolve host`) appears in `stderr`.
- **`stdin` (input)**: Supported via input redirection from a temp file. POSIX-compatible.
- **`stdout` / `stderr`**: The Daytona API merges stdout and stderr, so `exec()` writes each stream to a temporary file under `/tmp` and returns them separately. In single-service mode the image needs `base64` and a writable `/tmp` with room for the output. Single-service `exec()` also waits for any background process that still holds stdout or stderr open, so start daemons with their output redirected (for example `> /dev/null 2>&1 &`), or `exec()` will wait until the timeout. In DinD mode, as with Inspect's Docker sandbox, output a background process writes after the command exits is dropped.
- **Timeouts**: A command that exceeds its timeout raises `TimeoutError`, and it and its child processes are stopped. In DinD mode the command runs under `/usr/bin/timeout`, so the service image needs it (coreutils and BusyBox both provide it).
